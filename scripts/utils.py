...
def random_sq_bbox(img, mask_shape, image_size=256, margin=(16, 16)):
    ...
    # Next line changed by Rishi on Feb 14, 2025
    #mask[..., t:t+h, l:l+w] = 0 
    mask[..., t:t+h, :] = 0
...
def det_sq_bbox(img, mask_shape, image_size=256, margin=(16, 16)):
    """Generate a deterministic sqaure mask for inpainting
    """
    B, C, H, W = img.shape
    h, w = mask_shape
    margin_height, margin_width = margin
...
def det_spr_rec_bbox(img, mask_shape, image_size=256, margin=(16, 16)):
    """Generate a deterministic sqaure mask for inpainting
    """
    B, C, H, W = img.shape
    h, w = mask_shape
...
class mask_generator:
    def __init__(self, mask_type, mask_len_range=None, mask_prob_range=None,
        ...
        """
        assert mask_type in ['box', 'random', 'both', 'extreme', 'Missing_Wedge', 'random_mask', 'sparse_acq', 'none']
    ...
    def _retrieve_MW(self, img, ii):
        msk_r = self.mask_len_range[ii]
        #print('msk_r', msk_r)
        mask_h = int(msk_r)
        mask_w = int(msk_r)
        mask = det_sq_bbox(img, mask_shape=(mask_h, mask_w), image_size=self.image_size, margin=self.margin)
        return mask


    def _retrieve_Spr_Rec(self, img, ii):
        spr_rec_r = self.mask_len_range[ii]
        print('spr_rec_r', spr_rec_r)
        spr_rec_h = int(spr_rec_r)
        spr_rec_w = int(spr_rec_r)
        mask = det_spr_rec_bbox(img, mask_shape=(spr_rec_h, spr_rec_w), image_size=self.image_size, margin=self.margin)
        return mask
    ...
    def _retrieve_random_mask(self, img):    

        # random row-wise pixel sampling
        l, h = self.mask_prob_range
        prob = np.random.uniform(l, h)
        fract = int(prob * self.image_size)
        fract_sampl = random.sample(range(self.image_size), fract)
        mask_vec = torch.ones([1, self.image_size, self.image_size])
        mask_vec[0, fract_sampl, :] = 0 
        mask_b = mask_vec.view(1, self.image_size, self.image_size)
        mask_b = mask_b.repeat(3, 1, 1)
        mask = torch.ones_like(img, device=img.device)
        mask[:, ...] = mask_b
    ...
    def __call__(self, img, ii=None):
        if self.mask_type == 'random':
            ...
            return mask
        elif self.mask_type == 'Missing_Wedge':
            print('ii', ii)
            mask = self._retrieve_MW(img, ii)
            return mask
        elif self.mask_type == 'sparse_acq':
            mask = self._retrieve_Spr_Rec(img, ii)
            return mask
            ...
            mask = self._retrieve_random_mask(img)
            return mask
