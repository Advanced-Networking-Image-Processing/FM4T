...
yaml_path = "/homes/sruban/latent-diffusion-inpainting_sino_trial/ldm/models/ldm/inpainting_big/config_rl.yaml"
model_path = "/homes/sruban/latent-diffusion-inpainting_sino_trial/logs_ldm_trained_dummyAE/ldm_trained_org_phy123_losses_w_gan/real_only/act_epchs/checkpoints/epoch=000114.ckpt"
...
def generate_sino_mask(load_image_i, f_i):
    ...
    fract = int(512 * f_i)
    fract_sampl = random.sample(range(im_size[0]), fract)
    mask[fract_sampl, :] = 255
...
for ff in range(len(f)):
    ...
    for ii in range(n_images):
        ...
        c = model.cond_stage_model.encode(masked_image_tensor)

        # the mask is first downsampled
        cc = torch.nn.functional.interpolate(mask_tensor, size=c.shape[-2:])
        ...
        #print('x_samples_ddim.device', x_samples_ddim.device)
        
        # Insert the blending steps over here (Oct23, 2024)
        print('min and max of image_tensor', torch.min(image_tensor), torch.max(image_tensor))
        print('min and max of x_samples_ddim', torch.min(x_samples_ddim), torch.max(x_samples_ddim))
    ...
    save_path = 'results/Jiaze_data/w_blending/ldm_real_only'
    dxchange.write_tiff(pred_data, save_path + '/random_masking/maskr' + str(f_i) + '/pred_data', dtype='float32', overwrite=True)
    dxchange.write_tiff(inp_data, save_path + '/random_masking/maskr' + str(f_i) + '/input_data', dtype='float32', overwrite=True) 
    dxchange.write_tiff(mask_data, save_path + '/random_masking/maskr' + str(f_i) + '/mask_data', dtype='float32', overwrite=True)
    dxchange.write_tiff(inpainted_data, save_path + '/random_masking/maskr' + str(f_i) + '/inpainted_data', dtype='float32', overwrite=True)
    dxchange.write_tiff(mask_img_data, save_path + '/random_masking/maskr' + str(f_i) + '/masked_image_data', dtype='float32', overwrite=True)
    dxchange.write_tiff(blended_data, save_path + '/random_masking/maskr' + str(f_i) + '/blended_data', dtype='float32', overwrite=True) 
