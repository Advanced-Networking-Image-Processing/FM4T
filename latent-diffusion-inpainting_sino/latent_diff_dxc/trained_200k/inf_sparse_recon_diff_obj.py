...
for ii in range(len(fr_smb)):
    ...
    for jj in range(len(spr_sam)):
        ...
        dxchange.write_tiff(image_dxc, 'fine_tuned_20k/Spr_Rec/diff_size_obj/' + smb[ii] + '_obj/' + 'f_' + str(spr_sam[jj]) + '/org_image' + str(fr_smb[ii]), dtype='float32', overwrite=True)
        dxchange.write_tiff(mi_dxc, 'fine_tuned_20k/Spr_Rec/diff_size_obj/' + smb[ii] + '_obj/' + 'f_' + str(spr_sam[jj]) + '/masked_org_image' + str(fr_smb[ii]), dtype='float32', overwrite=True)
        dxchange.write_tiff(o_dxc, 'fine_tuned_20k/Spr_Rec/diff_size_obj/' + smb[ii] + '_obj/' +  'f_' + str(spr_sam[jj]) + '/output_image' + str(fr_smb[ii]), dtype='float32', overwrite=True)
