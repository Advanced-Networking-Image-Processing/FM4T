...
class VQLPIPSWithDiscriminatorSino(nn.Module):
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx,
        ...
        '''
        # Chunk of code for Sanity check
        inp_obj_det = np.array(inp_obj.detach().cpu())
        dxchange.write_tiff(inp_obj_det, 'image_samples_Nov7_24/rec_obj_ii_in_loss_Nov6_24', dtype='float32', overwrite=True)
        blablabla
        '''
        ...
        if optimizer_idx == 1:
            ...
            logits_real = torch.tensor(0.0, requires_grad=True)
            logits_fake = torch.tensor(0.0, requires_grad=True)
