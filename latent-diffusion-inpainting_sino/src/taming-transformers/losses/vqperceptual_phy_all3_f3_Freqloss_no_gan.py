...
def second_deg_loss(inputs, reconstructions):
    ...
    '''
    dxchange.write_tiff(rec_2_1x.cpu().numpy(), 'images/hessian/rec_2_1x', dtype='float32', overwrite=True)
    dxchange.write_tiff(ip_2_1x.cpu().numpy(), 'images/hessian/ip_2_1x', dtype='float32', overwrite=True)
    '''
...
def sino_2sino(inputs, reconstructions, ramp_filt, volume, radon, radon1):
    n_angs = 512; image_size = 512; loss_factor2 = 1e3
    ...
    '''
    dxchange.write_tiff(rec_obj1_h1.cpu().numpy(), 'images/sino2sino/rec_obj1_h1', dtype='float32', overwrite=True)
    dxchange.write_tiff(rec_obj1_h2v.cpu().numpy(), 'images/sino2sino/rec_obj1_h2v', dtype='float32', overwrite=True)
    '''
...
'''
def freq_lpips(self, inputs, reconstructions, ramp_filt, volume, radon):
    n_angs = 512; image_size = 512; loss_fact = 1
    
    inputs1 = inputs.view(-1, image_size, n_angs)
    recons1 = reconstructions.view(-1, image_size, n_angs)
    
    # LPIPS in the fft of the sinograms
    inputs_fft = torch.abs(torch.fft.fft2(inputs.contiguous(), norm="ortho"))
    recons_fft = torch.abs(torch.fft.fft2(reconstructions.contiguous(), norm="ortho"))
    
    lp_fr_sino = self.perceptual_loss(inputs_fft.contiguous(), recons_fft.contiguous())
    #print('lp_fr_sino', lp_fr_sino)
    
    # LPIPS in the fft of the recons
    pad = 512 # for 512 x 512 image size
    padded_input = torch.nn.functional.pad(inputs1, (0, pad, 0, 0))
    padded_recon = torch.nn.functional.pad(recons1, (0, pad, 0, 0))
    ...
    fr_inp_obj = torch.abs(torch.fft.fft2(radon.backward(filt_sino_inpf.contiguous()), norm="ortho").contiguous())
    fr_rec_obj_a = radon.backward(filt_sino_recf.contiguous())
    fr_rec_obj_b = torch.fft.fft2(fr_rec_obj_a.contiguous(), norm="ortho")
    fr_rec_obj = torch.abs(fr_rec_obj_b.contiguous())    
    
    lp_fr_obj = self.perceptual_loss(fr_inp_obj.contiguous(), fr_rec_obj.contiguous())
    #print('lp_fr_obj', lp_fr_obj)
    
    lp_fr_tot = torch.mean(lp_fr_sino + lp_fr_obj)
    
    del inputs1, recons1, inputs_fft, recons_fft, padded_input, padded_recon, inp_fft, rec_fft, filtered_inp, filtered_rec, filt_sino_rec, filt_sino_recf
    del filt_sino_inp, filt_sino_inpf
...
def Rec_obj(inputs, reconstructions, ramp_filt, volume, radon):
    ...
    n_angs = 512; image_size = 512; loss_factor = 1e5
    ...
    '''
    dxchange.write_tiff(inp_obj.cpu().numpy(), 'images/recon/inp_obj', dtype='float32', overwrite=True)
    dxchange.write_tiff(rec_obj.cpu().numpy(), 'images/recon/rec_obj', dtype='float32', overwrite=True)
    '''
    loss_rec_obj = loss_factor * torch.mean(torch.square(rec_obj - inp_obj))
    
    del filt_sino_recf, filt_sino_inpf, filt_sino_rec, filt_sino_inp, filtered_inp, filtered_rec, inp_fft, rec_fft, padded_input, padded_recon
    del loss_factor, n_angs, image_size, inputs1, recons1, pad

    return loss_rec_obj, inp_obj, rec_obj
...
class VQLPIPSWithDiscriminatorSino(nn.Module):
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx,
        ...
        eps_ip = (5e-10) * torch.rand(inp_sz, device=torch.device('cuda')) + (5e-10)
        
        recons_sz = list(reconstructions.shape)
        eps_rec = (5e-10) * torch.rand(recons_sz, device=torch.device('cuda')) + (5e-10)
        ...
        if torch.isnan(nll_loss):
            ...
            nll_loss = torch.tensor([1e-10], device=torch.device('cuda'))
        else:
            #print('loss_sec_deg is not nan')
            nll_loss = nll_loss + 1e-10
        ...
        loss_rec_obj, inp_obj, rec_obj = Rec_obj(inputs, reconstructions, ramp_filt, volume, radon)
        
        # Sino_2Sino loss
        sino_loss = sino_2sino(inputs, reconstructions, ramp_filt, volume, radon, radon1)
        
        # Freq domain loss with LPIPS
        inputs_fft = torch.abs(torch.fft.fft2(inputs, norm="ortho"))
        recons_fft = torch.abs(torch.fft.fft2(reconstructions, norm="ortho"))
        lp_fr_sino = self.perceptual_loss(inputs_fft.contiguous(), recons_fft.contiguous())

        inp_obj_fft = torch.abs(torch.fft.fft2(inp_obj, norm="ortho"))
        rec_obj_fft = torch.abs(torch.fft.fft2(rec_obj, norm="ortho"))
        lp_fr_obj = self.perceptual_loss(inp_obj_fft.contiguous(), rec_obj_fft.contiguous())

        freq_losses = torch.mean(lp_fr_sino + lp_fr_obj)
        
        del ramp_filt, radon, radon1, volume, inputs_fft, recons_fft, inp_obj_fft, rec_obj_fft
        ...
        if optimizer_idx == 0:
            ...
            loss = nll_loss + loss_sec_deg + sino_loss + loss_rec_obj + freq_losses + (d_weight * disc_factor * g_loss) + (self.codebook_weight * codebook_loss1.mean())
            ...
            log = {"{}/total_loss".format(split): loss.clone().detach().mean(), 
                   ...
                   "{}/freq_losses".format(split): freq_losses.detach().mean(),
