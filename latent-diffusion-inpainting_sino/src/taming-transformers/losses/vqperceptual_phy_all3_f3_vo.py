...
def Vol_fn(inputs, reconstructions, mask_data):
    
    vol_fact = 0.05
    #vol_fact = 0.0005  #Results are in trial2_close folder
    #print('mask_data size', mask_data.size())
    flip_inputs_lr = torch.fliplr(inputs); flip_recons_lr = torch.fliplr(reconstructions)
    _inputs_t = torch.cat((inputs, flip_inputs_lr), dim=2) 
    _recons_t = torch.cat((reconstructions, flip_recons_lr), dim=2)
    #print('_inputs_t size', _inputs_t.size())
    #print('_recons_t size', _recons_t.size())
    _inputs_fft = torch.abs(torch.fft.fftshift(torch.fft.fft2(_inputs_t))) * mask_data
    _recons_fft = torch.abs(torch.fft.fftshift(torch.fft.fft2(_recons_t))) * mask_data
    Vol_loss = torch.abs(torch.mean(_inputs_fft - _recons_fft))
    #Vol_loss = torch.mean(_inputs_fft) - torch.mean(_recons_fft)
    Vol_loss = Vol_loss * vol_fact 
...
class VQLPIPSWithDiscriminatorSino(nn.Module):
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx,
        ...
        mask_data1a = np.load('/homes/sruban/nicky_LDM_Torch_Radon/latent-diffusion-inpainting/src/taming-transformers/taming/modules/losses/mask_512_512.npy')
        #mask_data1a = np.transpose(np.dstack((mask_data1, mask_data1, mask_data1)), axes=[2, 0, 1])
        mask_data = torch.tensor(mask_data1a, dtype=torch.float32, device=torch.device('cuda'))
        mask_data = mask_data.view(1, 1, 1024, -1)
        #mask_data = mask_data.view(1, 3, 1024, -1)
        #print('mask_data size()', mask_data.size())

        #Nghia_T_Vo loss from 2014 paper (Tekin - Berkeley)
        Vol_loss = Vol_fn(inputs, reconstructions, mask_data)
        #print('Vol_loss', Vol_loss)
        ...
        if optimizer_idx == 0:
            ...
            loss = nll_loss + loss_sec_deg + sino_loss + loss_rec_obj + Vol_loss + (d_weight * disc_factor * g_loss) + (self.codebook_weight * codebook_loss1.mean())
            ...
            log = {"{}/total_loss".format(split): loss.clone().detach().mean(),
                   ...
                   "{}/Vo_loss".format(split): Vol_loss.detach().mean(),
                   "{}/p_loss".format(split): p_loss.detach().mean(),
                   "{}/d_weight".format(split): d_weight.detach(),
                   "{}/disc_factor".format(split): torch.tensor(disc_factor),
                   "{}/g_loss".format(split): g_loss.detach().mean(),
