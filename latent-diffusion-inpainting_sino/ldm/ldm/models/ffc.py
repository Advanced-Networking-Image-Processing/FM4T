...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        ffted = ffted.reshape(batch, -1, h, w // 2 + 1)
        ...
        output = torch.fft.irfft2(ffted, s=(h, w), dim=(-2, -1), norm='ortho')
...
if __name__ == '__main__':
    in_channels = 1
    out_channels = 1
    fourier_unit = FourierUnit(in_channels, out_channels)
    
    batch_size = 2
    height = 18
    width = 32
    input_tensor = torch.randn(batch_size, in_channels, height, width)
    output_tensor = fourier_unit(input_tensor)
