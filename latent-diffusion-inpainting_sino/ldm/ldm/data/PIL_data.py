...
class InpaintingTrain_autoencoder(Dataset):
    ...
    def generate_sino_mask(self, im_size):
        
        print('I am here in this function !!!')
        mask = np.zeros((im_size[0], im_size[1], 1), dtype=np.uint8)
        print('mask shape in generate_sino_mask function', mask.shape)
       
        fract = random.uniform(0.1, 0.9)
        fract_int = int(fract * im_size[0])

        sixty_percnt = random.sample(range(im_size[0]), fract_int)
        mask[sixty_percnt, :, :] = 255
    ...
    def __len__(self):
        print('total images is ', 200000)
        return 200000
        #return len(self.images)

    def __getitem__(self, i):
        
        #print('i in PIL_data', i)
        #print('self.data_root', self.data_root)
        ''' 
        ...
        f = h5py.File(self.data_root+"/ds-simu-tr1-sino.h5", 'r')
        images = f["sino"]
        image = images[i]
        ...
        #dxchange.write_tiff(image, 'dxchange_data/sino_sample', dtype='float32', overwrite=True)
        image = torch.from_numpy(image)
        
        #Need to correct from here
        mask = self.generate_sino_mask([self.size, self.size])
        print(mask.shape)
        #dxchange.write_tiff(mask, 'dxchange_data/sino_mask', dtype='float32', overwrite=True)
        
        mask[mask < 0.5] = 0
        mask[mask >= 0.5] = 1
        # mask = mask[None].transpose(0,3,1,2) (This line is only useful during inference.)
        ...
        if random.uniform(0, 1)<0.5:
            ...
            print('random.uniform is less than 0.5 !!!')
        else:
            batch = {"masked_image": np.squeeze(image,0), "image": np.squeeze(masked_image,0)}
            print('random.uniform is greater than 0.5 !!!')
        ...
        #print('batch["masked_image"].shape', batch["masked_image"].shape)
...
class InpaintingTrain_ldm(Dataset):
    ...
    def __getitem__(self, i):
        ...
        '''
        #print('i', i)
        mask_address = self.data_root + '/mask/ds-simu-tr1-sino_mask.h5'
        mask_al = h5py.File(mask_address, 'r')
        mask_all = mask_al["mask"] 
        mask = mask_all[i]
        ...
        #print('mask shape in generate_sino_mask function', mask.shape)
        fract = random.uniform(0.1, 0.9)
        fract_int = int(fract * im_size)
        sixty_percnt = random.sample(range(im_size), fract_int)
        mask[sixty_percnt, :, :] = 255
        ...
        image_address = self.data_root + '/sino/ds-simu-tr1-sino.h5'
