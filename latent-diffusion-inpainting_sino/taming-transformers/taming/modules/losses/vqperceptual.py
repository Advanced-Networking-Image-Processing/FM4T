...
def second_deg_loss(inputs, reconstructions):
    ip_1 = kornia.filters.spatial_gradient(inputs.contiguous())
    ...
    rec_1 = kornia.filters.spatial_gradient(reconstructions.contiguous())
    ...
    diff_2 = torch.square(rec_2_1x - ip_2_1x) + torch.square(rec_2_1y - ip_2_1y)
    loss_sec_deg = torch.mean(torch.sqrt(diff_2))
...
def Rec_obj(inputs, reconstructions):
    ...
    angles = np.linspace(0, np.pi, n_angs, endpoint=False)
    radon = tr.ParallelBeam(image_size, angles)

    fbp_ip = radon.backward(inputs)
    fbp_rec = radon.backward(reconstructions)
    print(fbp_ip.shape)
    print(fbp_rec.shape)
    blabla

    loss_rec_obj = torch.mean(torch.abs(fbp_ip - fbp_rec))
...
class VQLPIPSWithDiscriminatorSino(nn.Module):
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx,
        ...
        if optimizer_idx == 0:
            ...
            disc_factor = adopt_weight(self.disc_factor, global_step, threshold=self.discriminator_iter_start)
            loss = nll_loss + loss_sec_deg + loss_rec_obj + (d_weight * disc_factor * g_loss) + (self.codebook_weight * codebook_loss.mean())
