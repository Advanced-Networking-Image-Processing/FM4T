...
class DDIMSampler(object):
    ...
    def resample_sampling(self, measurement, measurement_cond_fn, cond, shape, operator_fn=None,
        ...
        print('img shape in line 218 of DDIM.resample_sampling class', img.shape)
        ...
        '''
        print('alphas size', alphas)
        print('alphas_prev size', alphas_prev)
        print('betas size', betas)
        ...
        for i, step in enumerate(iterator):   
            ...
            # The next 4 lines is pretty much useless.
            if mask is not None:
            ...
            out, pred_x0, pseudo_x0 = self.p_sample_ddim(img, cond, ts, index=index, use_original_steps=ddim_use_original_steps,
                                      ...
                                      unconditional_conditioning=unconditional_conditioning)
            ''' 
            print('out min and max', torch.min(out), torch.max(out))
            print('pred_x0 min and max', torch.min(pred_x0), torch.max(pred_x0))
            print('pseudo_x0 min and max', torch.min(pseudo_x0), torch.max(pseudo_x0))
            print('out, pred_x0, pseudo_x0 shapes', out.shape, pred_x0.shape, pseudo_x0.shape)
            print('measurement shape is (1, 3, 512, 512)', measurement.shape)
            
            out1 = out.detach().cpu().squeeze().numpy()
            out2 = (np.transpose(out1, (1, 2, 0)) + 1)/2
            dxchange.write_tiff(skimage.color.rgb2gray(out2), 'out_data', dtype='float32', overwrite=True)
 
            pred_x0_1 = pred_x0.detach().cpu().squeeze().numpy()
            pred_x0_2 = (np.transpose(pred_x0_1, (1, 2, 0)) + 1)/2
            dxchange.write_tiff(skimage.color.rgb2gray(pred_x0_2), 'pred_x0_data', dtype='float32', overwrite=True)

            pseudo_x0_1 = pseudo_x0.detach().cpu().squeeze().numpy()
            pseudo_x0_2 = (np.transpose(pseudo_x0_1, (1, 2, 0)) + 1)/2
            dxchange.write_tiff(skimage.color.rgb2gray(pseudo_x0_2), 'pseudo_x0_data', dtype='float32', overwrite=True)
            
            ms1 = np.clip(measurement.detach().cpu().squeeze().numpy(), -1, 1)
            ms2 = (np.transpose(ms1, (1, 2, 0)) + 1)/2
            dxchange.write_tiff(skimage.color.rgb2gray(ms2), 'measurment_data', dtype='float32', overwrite=True)
            blabla
            '''
            ...
            '''
            img1 = np.clip(img.detach().cpu().squeeze().numpy(), -1, 1)
            img2 = (np.transpose(img1, (1, 2, 0)) + 1)/2
            dxchange.write_tiff(skimage.color.rgb2gray(img2), 'img_data', dtype='float32', overwrite=True)
            print('img shape', img.shape)
            blabla
            '''
            ...
            if index <= (total_steps - index_split) and index > 0:   
                ...
                print('index', index)
                print('time travel steps !!!')
                #print('index in line #276', index)
                ...
                if index % 10 == 0 :  
                    ...
                    for k in range(i, min(i+inter_timesteps, len(list( reversed(timesteps) ))-1)):
                        ...
                        img, pred_x0, pseudo_x0 = self.p_sample_ddim(img, cond, ts_, index=index_, use_original_steps=ddim_use_original_steps,
                                            ...
                                            unconditional_guidance_scale=unconditional_guidance_scale,
                                            unconditional_conditioning=unconditional_conditioning)
                        #print('img in line #320 of DDIM.py', img.shape)
                        #blabla
                    ...
                    # Latent-based optimization for third stage
                        ...
                        print('optimization in latent space')
                        #print('I am in line #325')
