...
class DDIMSampler(object):
        ...
        for i, step in enumerate(iterator):   
            ...
            if index < phy_steps:   
                ...
                if index % 20 == 0:
                    ...
                    if index >= 0:
                        sigma = gamma*((1 - a_prev) / a_t) * (1 - (a_t / a_prev))  
                    #else:
                    #    sigma = 0.5
                    ...
                    if i >= 0:
                        ...
                        ub_opt_var = torch.quantile(opt_var, 0.99)
                        lb_opt_var = torch.quantile(opt_var, 0.01)
                        img_p_t = (ub_opt_var - lb_opt_var) * ((img_p_t - torch.quantile(img_p_t, 0.01)) / (torch.quantile(img_p_t, 0.99) - torch.quantile(img_p_t, 0.01)))
    ...
    def pixel_optimization(self, measurement, x_prime, operator_fn, eps=1e-4, max_iters=200):
        """
        ...
        rho = 1e-4 # rho changed

        angles_a = np.linspace(0, np.pi, 256, endpoint=False)
        angles = torch.tensor(angles_a)

        # Training loop
        for _ in range(max_iters):
            optimizer.zero_grad()
            measurement_ta = measurement.permute(0,2,3,1)
            sino_opt_var = ct_parallel_project_2d_batch(opt_var.permute(0,2,3,1), angles)
            
            print('measurement_ta shape', measurement_ta.shape)
            print('sino_opt_var shape', sino_opt_var.shape)

            measurement_loss = loss(measurement_ta, sino_opt_var) + ((rho/2)*loss(x_prime, opt_var))
