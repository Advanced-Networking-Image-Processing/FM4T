...
from ldm.models.diffusion.ddim_admm_match_sf1_diff_phy_guid.ddim_git_admm_match_sf1_p1e_4_pg10_inv_prb_not_matched import DDIMSampler
...
parser.add_argument('--save_dir', type=str, default='./results_512/sinogram/samples_psmc/p1e_4_pg10/LDM_fine_tuned_admm_not_matched/MW')
...
for i, ref_img in enumerate(loader):
    ...
    if (measure_config['operator'] ['name'] == 'inpainting') and (measure_config['mask_opt']['mask_type'] == 'Missing_Wedge'):
      ...
      if len(msk_lr) > 2:
        for jj in range(len(msk_lr)):
          ...
          print('m/s config', measure_config['operator']['name'])
          ...
          print('y shape is (1, 3, 512, 512)', y.shape)
          print('y_n shape is (1, 3, 512, 512)', y_n.shape)
 
          # Sampling
          samples_ddim, _ = sample_fn(measurement=y_n)
          # The samples_ddim shape is (1, 3, 128, 128)
